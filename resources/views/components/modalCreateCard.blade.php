<div id="modalCreateCard" class="fixed inset-0 flex items-center justify-center p-4 z-50 hidden" style="display: none;">
...
@push('scripts')
    <script>
        ...
        $('body').on('click', '#btn-add-card', function() {
            ...
            $('#modalCreateCard').removeClass('hidden').css('display', 'flex');
        });

        // Event untuk menutup modal create card
        $('#closeModalCreateCard').click(function() {
            $('#modalCreateCard').addClass('hidden').css('display', 'none');
        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        const card = response.card; // Now this contains all properly formatted data
                        const parentList = $(`.parent-list-item[data-parent-id="${card.parent_id}"]`);

                        if (parentList.length) {
                            let cardsContainer = parentList.find('.space-y-3');
                            if (cardsContainer.length === 0) {
                                parentList.append('<div class="space-y-3" id="card-container"></div>');
                                cardsContainer = parentList.find('.space-y-3');
                            }

                            const cardHtml = `
                <div class="bg-gray-600 p-3 rounded-md mb-2 card-item outline-2 outline-orange-500/80" data-card-id="${card.id}">
                    ...
                    <div class="flex justify-between items-center mt-1">
                        <p class="text-xs text-gray-400">Due: ${card.format_date || 'No due date'}</p>
                        <p class="text-xs ${card.status === 'completed' ? 'text-green-300' : (card.status === 'late' ? 'text-red-300' : 'text-yellow-300')}">
                            ${card.status_text || ''}
                        </p>
                         <button class="btn-move-card cursor-pointer hover:text-orange-400 transition duration-200" data-id="${card.id}">
                                         Move
                                    </button>
                    </div>
